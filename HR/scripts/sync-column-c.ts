...
async function main() {
  ...
  try {
    ...
    // Build normalized name map from Excel - use __EMPTY_2 (Column C)
    const excelNameMap = new Map<string, string>();
    for (const row of excelData) {
      const namaAsesor = row['__EMPTY_2']; // Column C
    ...
    console.log(`Excel unique asesor names (from Column C): ${excelNameMap.size}`);
